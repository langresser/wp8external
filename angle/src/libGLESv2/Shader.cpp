...
void Shader::setBinary(unsigned char* data, int size)
{
	if (m_binary)
	{
		delete[] m_binary;
	}
	m_binary = new unsigned char[size];
	memcpy(m_binary, data, size);
	m_size = size;
}

unsigned char* Shader::getBinary() const
{
	return m_binary;
}

bool Shader::isBinary() const
{
	return m_binary != nullptr;
}

int Shader::getBinarySize() const
{
	return m_size;
}
