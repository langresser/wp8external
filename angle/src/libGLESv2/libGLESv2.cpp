...
void __stdcall glTranslateProgram(GLuint program, const char* output)
{
	EVENT("(GLuint program = %d)", program);
...
void __stdcall glShaderBinary(GLsizei n, const GLuint* shaders, GLenum binaryformat, const GLvoid* binary, GLsizei length)
{
    ...
    try
    {
		...
		if (context)
		{
			gl::Shader *shader = context->getShader(*shaders);
			if (!shader)
			{
				return;
			}
			shader->setBinary((unsigned char*)binary, length);
		}
    }
    catch(std::bad_alloc&)
    {
