...
bool ProgramBinary::link(InfoLog &infoLog, const AttributeBindings &attributeBindings, FragmentShader *fragmentShader, VertexShader *vertexShader)
{
	...
	mVertexExecutable = mRenderer->loadExecutable((void *)vertexShader->getBinary(), vertexShader->getBinarySize(), rx::SHADER_VERTEX);
	mPixelExecutable = mRenderer->loadExecutable((void*)fragmentShader->getBinary(), fragmentShader->getBinarySize(), rx::SHADER_PIXEL);
	return true;
}
