...
EGLint SwapChain11::resetOffscreenTexture(int backbufferWidth, int backbufferHeight)
{
    ...
    else
    {
        ...
        offscreenTextureDesc.MiscFlags = mWindow == nullptr ? D3D11_RESOURCE_MISC_SHARED : 0;
...
EGLint SwapChain11::reset(int backbufferWidth, int backbufferHeight, EGLint swapInterval)
{
    ...
    if (mWindow != nullptr)
    {
        IDXGIFactory2 *factory = (IDXGIFactory2 *)mRenderer->getDxgiFactory();

#if WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP
		DXGI_SWAP_CHAIN_DESC1 swapChainDesc = {0};
		swapChainDesc.Width = 0;                                     // use automatic sizing
		swapChainDesc.Height = 0;
		swapChainDesc.Format = gl_d3d11::ConvertRenderbufferFormat(mBackBufferFormat);           // this is the most common swapchain format
...
#else
		Windows::UI::Core::CoreWindow^ window = mWindow.Get();
...
unsigned char* SwapChain11::getShaderData(const char* pszFileName, unsigned long * pSize)
{
	...
	*pSize = 0;
	do
	{
		...
		auto localFolderPath = Windows::ApplicationModel::Package::Current->InstalledLocation->Path + "\\Assets\\";
		size_t convertedChars = 0;
		int len = localFolderPath->Length();
		char * pszUtf8Str = new char[len*3 + 1];
		WideCharToMultiByte(CP_UTF8, 0, localFolderPath->Data(), len+1, pszUtf8Str, len*3 + 1, 0, 0);
		basePath = pszUtf8Str;
#endif
		basePath += pszFileName;
...
void SwapChain11::initPassThroughResources()
{
	...
	unsigned long vsSize = 0;
	unsigned char* vsShader = getShaderData("../pass_through_vert.cso", &vsSize);
	unsigned long fsSize = 0;
	unsigned char* fsShader = getShaderData("../pass_through_frag.cso", &fsSize);
    result = device->CreateInputLayout(quadLayout, 2, vsShader, vsSize, &mPassThroughIL);
    ASSERT(SUCCEEDED(result));
    d3d11::SetDebugName(mPassThroughIL, "Swap chain pass through layout");

    result = device->CreateVertexShader(vsShader, vsSize, NULL, &mPassThroughVS);
    ASSERT(SUCCEEDED(result));
    d3d11::SetDebugName(mPassThroughVS, "Swap chain pass through vertex shader");

    result = device->CreatePixelShader(fsShader, fsSize, NULL, &mPassThroughPS);
    ASSERT(SUCCEEDED(result));
